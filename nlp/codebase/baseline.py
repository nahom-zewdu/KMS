...
class CodebaseBaselineSync:
    ...
    def _index_file(self, gh_file, repo_id: str, repo_full_name: str):
        """Index single file with module_path."""
        ...
        # Create PART_OF relationship (file → repository)
        self._create_part_of_edge(repo_id, file_path)
    ...
    def _infer_module_type(self, module_path: str) -> str:
        ...
        if any(k in lower for k in ["api", "handlers", "routes"]):
            return "api"
        if any(k in lower for k in ["nlp", "engine", "worker", "ingestion"]):
            return "core"
        if any(k in lower for k in ["utils", "common", "helper", "test"]):
            return "utils"
        return "feature"
